...
import br.com.ufmg.spring.jpa.Commit;
import br.com.ufmg.spring.jpa.Projeto;
		...
		try {
			...
			while (i.hasNext()) {				
				...
				if (currentCommit.getParentCount() == 1) {
					...
					commit.setAutor(currentCommit.getAuthorIdent().getName());
					commit.setData(currentCommit.getAuthorIdent().getWhen());
					commit.setProjeto(new Projeto(8));
					
					this.handler.handleCommit(commit);
					...
					for (Refactoring ref : refactoringsAtRevision) {						
						br.com.ufmg.spring.jpa.Refactoring refactoring = new br.com.ufmg.spring.jpa.Refactoring();
						refactoring.setCommit(commit);
						refactoring.setTipo(ref.getName());
						this.handler.handleRefactoring(refactoring);
					}
